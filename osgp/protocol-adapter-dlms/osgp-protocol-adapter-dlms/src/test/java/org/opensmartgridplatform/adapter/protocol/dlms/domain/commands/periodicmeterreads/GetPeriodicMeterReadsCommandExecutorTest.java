...
class GetPeriodicMeterReadsCommandExecutorTest {
  ...
  @Mock private GetPeriodicMeterReadsGasCommandExecutor gasCommandExecutor;
  ...
  //
  //    when(this.dlmsHelper.readDataObject(result0,
  // PERIODIC_E_METER_READS)).thenReturn(resultData);
  //
  //    when(this.dlmsHelper.getAndCheck(
  ...
  //        .thenReturn(Collections.singletonList(getResult));
  //
  //    when(this.dlmsHelper.readDataObject(eq(getResult),
  // any(String.class))).thenReturn(resultData);
  //
  ...
  //
  //    // Only 2 meterreads are expected. The 3rd meterread has a logtime outside the requested
  // period.
